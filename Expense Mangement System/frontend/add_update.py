...
def add_update_tab():
    ...
    with st.form(key='expense_form'):
        ...
        for i in range(5):
            with col1:
                amount_input = st.number_input(
                    ...
                    value=st.session_state.form_data[i]['amount'],
            ...
            with col2:
                category_input = st.selectbox(
                    label='Category', options=categories, format_func=lambda x: category_formatted[x],
                    index=categories.index(st.session_state.form_data[i]['category']),
                    key=f'category_{i}_{selected_date_str}',
                    label_visibility='collapsed'
                )
            with col3:
                notes_input = st.text_input(
                    label='Notes', value=st.session_state.form_data[i]['notes'],
        ...
        if submit_button:
            ...
            valid_expenses = [exp for exp in expenses if (exp['amount'] or 0) > 0 and exp['category'] != "Select Category"]
            ...
            if partially_filled:
                # Show a warning if there are partially filled entries
                ...
                if not valid_expenses and not existing_expenses:
                    st.error("🚨 **No valid expenses provided!** Please add at least one valid expense to proceed.")

                else:
                    # Send the request to the backend
                    try:
                        response = requests.post(f'{API_URL}/expenses/{selected_date_str}', json=valid_expenses)
                        if response.status_code == 200:
                            ...
                            st.error(
                                f'❌ Failed to update expenses: Server returned {response.status_code} - '
                                f'{response.json().get("message", "Unknown error")}. '
                                f'Details: {response.json().get("errors", "No additional details.")}'
                            )
