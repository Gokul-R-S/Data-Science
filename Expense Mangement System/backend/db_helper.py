...
def connect_db(commit=False):
    """Context manager for a MySQL cursor object , with optional commit."""
    connection = None
    cursor = None
    try:
...
def insert_expense(expense_date, amount, category, notes):
    ...
    with connect_db(commit=True) as cursor:
        ...
        logger.debug(f"Inserted expense | Date: {expense_date}, Amount: {amount}, Category: {category}, Notes: {notes}")
        logger.info(f"Expense inserted for {expense_date} under '{category}'")
